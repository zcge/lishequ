...
$(document).ready(function() {
	$(".Regional").click(function() {
		var navbarToTop = $(".screeningOnlySee").offset().top;
		$(".mui-content").animate({
			scrollTop: navbarToTop
		}, 0); //0是ms,也可以用slow代替 
...
$(".mui-content").scroll(function() {
	var s = $(".mui-content").scrollTop();
	var navbarToTop = $(".blankDiv").offset().top;
	if(s < navbarToTop) {
		$('.grade-eject').removeClass('grade-w-roll');
		$('.Category-eject').removeClass('grade-w-roll');
		$('.Sort-eject').removeClass('grade-w-roll');
		
		$('.screeningOnlySee').removeClass("mui-hidden");
		$('.screening').addClass("mui-hidden");
		//		alert(s);
	}else{
		$('.screeningOnlySee').addClass("mui-hidden");
		$('.screening').removeClass("mui-hidden");
	}
});
